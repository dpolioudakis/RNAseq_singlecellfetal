...
## Inputs
# DS-002 digital gene expression parent directory
inPntDir <- "../DS-002_DP/data/digital_gene_expression/Homo_sapiens.GRCh37.75.dna.primary_assembly_NoERCC/SxaQSEQsXbp083L1"
# DS-002 digital gene expression exon intron parent directory
inPntExInDir <- "../DS-002_DP/data/digital_gene_expression/Homo_sapiens.GRCh37.75.dna.primary_assembly_NoERCC_ExonIntron/SxaQSEQsXbp083L1"
...
inFilesL <- inFilesL[grep("dge_FtMm250.txt", inFilesL)]
...
# Add digital expression from exon + intron alignment
inPathsDF <- rbind(inPathsDF
  , paste0(inPntExInDir, "/", inDirsL, "/", "out_gene_exon_tagged_dge_FtMm250.txt"))
# Fluidigm HT-003
inHtDF <- read.table("../HT-003_DP/data/htseq/GRCh37.75_NoERCC/Exprs_HTSCexon_FtMm3e4_ens.txt", header= TRUE)
# Fluidigm HT-003 downsampled read depth
inHt08DF <- read.table("../HT-003_DP/data/htseq/GRCh37.75_NoERCC/downsampleReads/Exprs_HTSCexon_downSample08p_FtMm3e4_ens.csv", header= TRUE)
inHt49DF <- read.table("../HT-003_DP/data/htseq/GRCh37.75_NoERCC/downsampleReads/Exprs_HTSCexon_downSample49p_FtMm3e4_ens.csv", header= TRUE)

# Make output graphs directory
dir.create("../analysis/graphs", recursive = TRUE)

## Variables
graphCodeTitle <- "Number_Genes_Detected_Violin_Plots.R"
outGraphPfx <- "../analysis/graphs/Number_Genes_Detected_Violin_Plots"
...
colnames(nGenesDF) <- c("Exon", "Exon down-sample to 10%"
  , "Exon down-sample to 25%", "Exon down-sample to 50%", "Exon + Intron")
...
ggplot(ggDF, aes(x = SAMPLE, y = NUM_GENES, col = SAMPLE)) +
  geom_violin() +
  geom_text(data = mnDF
  ...
  ggtitle(paste0(graphCodeTitle
    ...
    , "\nFiltered cells with >250 counts mapping to mouse exons for each sample for DS-002"
    , "\nFiltered cells with >3e4 counts mapping to mouse for each sample for HT-003"
    , "\n"))
ggsave(paste0(outGraphPfx, ".pdf"), width = 8, height = 7)
################################################################################