...
DE_Filters_ExpMatrix <- function(
  ...
  cdf <- as.matrix(centSO@data)[ ,centSO@ident == clusterID1]
  # Expressed > 0 counts in > X% of cells in cluster
...
Format_DE <- function (deLM, clusterID1, clusterID2) {
  ...
  cdf <- as.matrix(centSO@data)[
    row.names(centSO@data) %in% deDF$Gene, centSO@ident == clusterID1]
  ...
  deDF$Percent_All <- (rowSums(as.matrix(centSO@data)[
    row.names(centSO@data) %in% deDF$Gene, ] > 0)
...
# Remove cluster 16
de_PW_DF <- de_PW_DF[de_PW_DF$Cluster2 != 16, ]

# Write to tab delimited table
print("Writing DE pairwise table to text file")
write.table(x = de_PW_DF
  , file = paste0(outTable, "PC1-40_Cluster", clusterID1, ".txt")
...
idx <- as.vector(sapply(de_PW_DFL, nrow) == length(unique(de_PW_DF$Cluster2)))
