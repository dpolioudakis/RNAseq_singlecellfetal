...
in_subclust_so_dir <- "../analysis/analyzed_data/Seurat_ClusterRound2/ClusterRound2/20190103/"
# clustering to use
...
in_subclust_de_dir <- "../analysis/tables/Seurat_ClusterRound2/DE/20181228/"
...
# human transcriptional regulators
tr_tb <- bind_rows(
  # TFs
  read_table("../source/AnimalTFDB_Homo_sapiens_TF_EnsemblID.txt"
    , col_names = FALSE)
  ...
  "../source/AnimalTFDB_Homo_sapiens_chr_remodeling_factor_EnsemblID.txt"
    , col_names = FALSE)
  # human co-factors
  , read_table("../source/AnimalTFDB_Homo_sapiens_cofactor_EnsemblID.txt"
    , col_names = FALSE)
)
...
main_function <- function(){
  ...
  # run_plot_de_tr_vs_subcluster_expression_heatmaps(cluster_col = cluster_col)
  plot_de_asd_vs_subcluster_expression_heatmaps(cluster_col = cluster_col)
  plot_de_epilepsy_vs_subcluster_expression_heatmaps(cluster_col = cluster_col)
  plot_de_id_vs_subcluster_expression_heatmaps(cluster_col = cluster_col)
  plot_de_genes_oRG_subcluster_intersected_with_shin_nsc_genes()
}
################################################################################
...
### TRs differentially expressed by sub-cluster

plot_de_tr_vs_subcluster_expression_heatmaps <- function(
  cluster_id, seurat_obj, noCent40kExM, cluster_col){
  ...
  gene_group_df <-
    subclust_de_tb %>%
    # subset to enrichment filters
    filter(Cluster == cluster_id & FDR < 0.05 & Log2_Fold_Change > 0.2) %>%
    # subset to TRs
    filter(Ensembl %in% pull(tr_tb)) %>%
    group_by(Subcluster) %>%
    arrange(Subcluster, Log2_Fold_Change) %>%
    select(Gene = Gene, Group = Subcluster) %>%
    as.data.frame()
  ...
  graph_height = 2 + nrow(gene_group_df)*0.25
  ...
  Plot_Marker_Genes_Heatmap_SetColWidths(
    ...
    script_name
    , "\n\nExpression of transcriptional regulators enriched for sub-clusters of cluster: "
    , cluster_annot, " (", cluster_id, ")"
    , "\nEnrichment determined by DE of cells in sub-cluster versus other in cluster"
    , "\nx-axis: Genes"
    , "\ny-axis: Cells ordered by cluster"
  ...
  ggsave(
    paste0(out_graph, "cluster", cluster_id
    , "_de_tr_vs_subclust_expression_heatmap_zscore.png")
  ...
  ggsave(
    paste0(out_graph, "cluster", cluster_id,
     "_de_tr_vs_subclust_expression_heatmap.png")
      , height = graph_height, width = 13, limitsize = FALSE)
}

run_plot_de_tr_vs_subcluster_expression_heatmaps <- function(cluster_col){

  print("run_plot_de_tr_vs_subcluster_expression_heatmaps")

  cluster_annot_tb$cluster_number %>%
    map(
      ~tryCatch({
          plot_de_tr_vs_subcluster_expression_heatmaps(
            seurat_obj = centSO
            , noCent40kExM = noCentExM
            , cluster_id = .
            , cluster_col = cluster_col
...
### Disease genes differentially expressed by sub-cluster

plot_de_genes_of_interest_vs_subcluster_expression_heatmaps <- function(
  cluster_id, seurat_obj, noCent40kExM, cluster_col, genes
  , gene_list_plot_title, out_graph_pfx){

  print("plot_de_genes_of_interest_vs_subcluster_expression_heatmaps")
  print(paste0("cluster: ", cluster_id))
  ...
  Plot_Marker_Genes_Heatmap_SetColWidths(
    ...
    script_name
    , "\n\nExpression of genes enriched for sub-clusters of cluster: "
    , cluster_annot, " (", cluster_id, ")"
    , gene_list_plot_title
    , "\nEnrichment determined by DE of cells in sub-cluster versus other in cluster"
    , "\nx-axis: Genes"
    , "\ny-axis: Cells ordered by cluster"
    , "\nNormalized expression, mean centered variance scaled by gene"
  ...
  ggsave(
    paste0(out_graph_pfx, "_zscore.png")
    , height = graph_height, width = 13, limitsize = FALSE)
  ...
  ggsave(
    paste0(out_graph_pfx, ".png")
      , height = graph_height, width = 13, limitsize = FALSE)
}

plot_de_asd_vs_subcluster_expression_heatmaps <- function(cluster_col){

  print("plot_de_asd_vs_subcluster_expression_heatmaps")

  cluster_annot_tb$cluster_number %>%
    map(
      ~tryCatch({
          plot_de_genes_of_interest_vs_subcluster_expression_heatmaps(
            seurat_obj = centSO
            , noCent40kExM = noCentExM
            , cluster_id = .
            , cluster_col = cluster_col
            , genes = pull(asd_tada_tb)
            , gene_list_plot_title = "ASD risk genes (Sander et al.)"
            , out_graph_pfx = paste0(out_graph, "cluster", .
              , "_de_asd_vs_subclust_expression_heatmap")
          )
        }
          , error = function(cond){
            message(paste0("Error for cluster: ", .))
...
plot_de_id_vs_subcluster_expression_heatmaps <- function(cluster_col){

  print("plot_de_id_vs_subcluster_expression_heatmaps")
    ...
    map(
      ~tryCatch({
          plot_de_genes_of_interest_vs_subcluster_expression_heatmaps(
            ...
            , cluster_id = .
            , cluster_col = cluster_col
            , genes = pull(id_genes_tb)
            , gene_list_plot_title = "ID risk genes"
            , out_graph_pfx = paste0(out_graph, "cluster", .
              , "_de_id_vs_subclust_expression_heatmap")
          )
        }
          , error = function(cond){
            message(paste0("Error for cluster: ", .))
...
plot_de_epilepsy_vs_subcluster_expression_heatmaps <- function(cluster_col){

  print("plot_de_epilepsy_vs_subcluster_expression_heatmaps")
    ...
    map(
      ~tryCatch({
          plot_de_genes_of_interest_vs_subcluster_expression_heatmaps(
            ...
            , cluster_id = .
            , cluster_col = cluster_col
            , genes = pull(epilepsy_tb)
            , gene_list_plot_title = "Epilepsy risk genes (Ruzzo)"
            , out_graph_pfx = paste0(out_graph, "cluster", .
              , "_de_epilepsy_vs_subclust_expression_heatmap")
          )
        }
          , error = function(cond){
            message(paste0("Error for cluster: ", .))
...
### plot DE genes between oRG sub-cluster 0 and 1 intersected with Shin genes

plot_de_genes_oRG_subcluster_intersected_with_shin_nsc_genes <- function(){

  print("plot_de_genes_oRG_subcluster_intersected_with_shin_nsc_genes")
  ...
  # genes associated with adult NSCs trajectory analysis
  shin_tfs <- read_csv(
    "../tmp/shin2015_table_s5.csv", skip = 2, col_names = TRUE) %>%
    clean_variable_names %>% mutate_all(toupper)
  shin_genes <- read_csv(
    "../tmp/shin2015_table_s4.csv", skip = 2, col_names = TRUE) %>%
    clean_variable_names %>% mutate_all(toupper)

  # DE genes between oRG sub-cluster 0 and 1
  fold_changes <- (noCentExM[ ,so@ident == 0] %>% rowMeans) - (noCentExM[ ,so@ident == 1] %>% rowMeans)

  # plot DE genes between oRG sub-cluster 0 and 1 intersected with Shin genes
  fold_changes[abs(fold_changes) > 0.25] %>%
    as.list %>%
    as_tibble() %>%
    gather() %>%
    arrange(value) %>%
    mutate(shin_genes = if_else(
        key %in% shin_genes$upregulated[1:150], "shin_up"
      , if_else(
        key %in% shin_genes$downregulated[1:150], "shin_down", NULL
      ))) %>%
    mutate(key = factor(key, levels = key)) %>%
    ggplot(aes(x = key, y = value, fill = shin_genes)) +
      geom_bar(stat = "identity") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(paste0(out_graph, "example_pairwise_de_shin_genes_barplot.png")
      , width = 30)

  # plot DE genes between oRG sub-cluster 0 and 1 intersected with Shin TFs
  fold_changes[abs(fold_changes) > 0.25] %>%
    as.list %>%
    as_tibble() %>%
    gather() %>%
    arrange(value) %>%
    mutate(shin_tfs = if_else(
        key %in% shin_tfs$up_tfs[1:150], "shin_up"
      , if_else(
        key %in% shin_tfs$down_tfs[1:150], "shin_down", NULL
      ))) %>%
    mutate(key = factor(key, levels = key)) %>%
    ggplot(aes(x = key, y = value, fill = shin_tfs)) +
      geom_bar(stat = "identity") +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    ggsave(paste0(out_graph, "example_pairwise_de_shin_tfs_barplot.png")
      , width = 30)
}
################################################################################
