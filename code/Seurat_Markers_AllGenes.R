...
## Variables
graphCodeTitle <- "Seurat_Markers_AllGenes_AllGenes.R"
outGraphPfx <- args[2]
outTablePfx <- args[3]
outRdatPfx <- args[4]
# outGraphPfx <- "../analysis/graphs/Seurat_Markers_AllGenes_exon_FtMm250_"
# outTablePfx <- "../analysis/tables/Seurat_Markers_AllGenes_exon_FtMm250_"
# outRdatPfx <- "../analysis/Seurat_Markers_AllGenes_exon_FtMm250_"
...
# the positive ones
# seuratO.markers <- FindAllMarkers(seuratO, only.pos = TRUE, min.pct = 0.25
#   , thresh.use = 0.25, return.thresh = 1)
markers <- FindAllMarkers(seuratO, only.pos = TRUE, min.pct = 0
  , min.diff.pct = 0, thresh.use = 0, return.thresh = 2)
