...
# DS-002-003-004-005-006: exDF and metDF
load("../analysis/Expression_Matrix_Compile_dge_FtMm250_DS-2-3-4-5-6.Rdata")
# Subsetting for testing
# idx <- sample(1:ncol(exDF), 1000)
# exDF <- exDF[idx]
# metDF <- metDF[idx, ]

## Variables
graphCodeTitle <- "Seurat_Cluster_DS-2-3-4-5-6.R"
outGraphPfx <- "../analysis/graphs/Seurat_Cluster_DS-2-3-4-5-6/Seurat_Cluster_DS-2-3-4-5-6_exon_FtMm250_RegLib_"
outTablePfx <- "../analysis/tables/Seurat_Cluster_DS-2-3-4-5-6/Seurat_Cluster_DS-2-3-4-5-6_exon_FtMm250_RegLib_"
outRdatPfx <- "../analysis/Seurat_Cluster_DS-2-3-4-5-6/Seurat_Cluster_DS-2-3-4-5-6_exon_FtMm250_RegLib_"
...
centSO <- Setup(centSO, min.cells = 3, min.genes = 200
  , do.logNormalize = T, total.expr = 1e4, project = "DS-005-006"
...
mito.genes <- grep("^MT-", rownames(centSO@data), value = T)
...
print(ncol(centSO@data))
filtersDF <- data.frame(
  FILTERS = c(as.character(filtersDF$FILTER), "> 2500 genes detected")
  , REMAINING = c(filtersDF$REMAINING, ncol(centSO@data)))
...
librarylab <- rep(0, length(centSO@data@Dimnames[[2]]))
dhgCells <- c(as.character(metDF$CELL)[metDF$SEQ_RUN == "DS-005-006" & metDF$NEXTERA == "N701"]
  , as.character(metDF$CELL)[metDF$SEQ_RUN == "DS-005-006" & metDF$NEXTERA == "N702"])
librarylab[centSO@data@Dimnames[[2]] %in% dhgCells] <- 1
centSO <- AddMetaData(centSO, percent.mito, "librarylab")

# No center or scale
noCentSO <- centSO
noCentSO <- RegressOut(centSO
  , latent.vars = c("nUMI", "percent.mito", "librarylab")
  , do.scale = FALSE, do.center = FALSE)
# Center and scale
...
save(centSO, noCentSO, file = paste0(outRdatPfx, "seuratO.Robj"))
################################################################################
...
centSO <- RunTSNE(centSO, dims.use = 1:40, do.fast = T)
pdf(paste0(outGraphPfx, "tSNE_Res0.6_200gd_2500dg_Mt5_PCA1to40.pdf"))
TSNEPlot(centSO, pt.size = 0.75)
dev.off()

centSO <- FindClusters(centSO, pc.use = 1:6, resolution = 0.6
  , print.output = 0, save.SNN = T)
centSO <- RunTSNE(centSO, dims.use = 1:6, do.fast = T)
...
centSO <- FindClusters(centSO, pc.use = 1:10, resolution = 0.6
  , print.output = 0, save.SNN = T)
centSO <- RunTSNE(centSO, dims.use = 1:10, do.fast = T)
...
ggDF$BRAIN[ggDF$BRAIN == 2] <- 3
ggDF$BRAIN[ggDF$BRAIN == 1] <- 2
metDF$LIBRARY <- "Plath"
...
ggB <- ggplot(ggDF, aes(x = tSNE_1, y = tSNE_2, col = BRAIN)) +
  ...
  ggtitle(paste0(graphCodeTitle
    ...
    , "\ntSNE PCA 1-10"
    , "\n"))
ggsave(paste0(outGraphPfx, "tSNE_Sample_200gd_2500dg_Mt5_PCA1to10.pdf")
  , width = 8, height = 6)
# ggplot Seq Run
ggS <- ggplot(ggDF, aes(x = tSNE_1, y = tSNE_2, col = SEQ_RUN)) +
  ...
  ggtitle(paste0(graphCodeTitle
    ...
    , "\ntSNE PCA 1-10"
    , "\n"))
ggsave(paste0(outGraphPfx, "tSNE_Lane_200gd_2500dg_Mt5_PCA1to10.pdf")
  , width = 8, height = 6)
# ggplot Region
...
pdf(paste0(outGraphPfx, "tSNE_Metadata_200gd_2500dg_Mt5_PCA1to10.pdf")
  , width = 8, height = 6)
ggC; ggB; ggR; ggS; ggL
dev.off()
# Save as grid
pdf(paste0(outGraphPfx, "tSNE_MetadataGrid_200gd_2500dg_Mt5_PCA1to10.pdf")
  , width = 12, height = 16)
MultiPlotList(list(ggC, ggB, ggS, ggL, ggR), cols = 2)
dev.off()
...
save(centSO, noCentSO, centSO.markers, file = paste0(outRdatPfx, "seuratO.Robj"))
...
# Subset to genes
ggDF <- centSO@scale.data[row.names(centSO@scale.data) %in% top10$gene, ]
# Order by clustering
...
ggDF <- melt(ggDF)
# Set gene order
ggDF$Var1 <- factor(ggDF$Var1, levels = rev(top10$gene))
...
ggplot(ggDF, aes(x = Var2, y = Var1, fill = value)) +
  ...
  ylab("Marker gene") +
  xlab("Cells ordered by clustering") +
  ggtitle(paste0(graphCodeTitle
    , "\n"
    , "\nTop 10 marker genes"
...
ggsave(paste0(outGraphPfx, "DoHeatmap_Top10Markers_CenterScale.png")
  , width = 10, height = 4 + 0.11*nrow(top10), dpi = 1200 )

# Subset to genes
ggDF <- noCentSO@scale.data[row.names(centSO@scale.data) %in% top10$gene, ]
# Order by clustering
idx <- match(colnames(ggDF), names(sort(centSO@ident)))
ggDF <- ggDF[ ,idx]
# Format for ggplot2
...
ggsave(paste0(outGraphPfx, "DoHeatmap_Top10Markers.png")
  , width = 10, height = 4 + 0.11*nrow(top10), res = 1200)
