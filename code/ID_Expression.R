...
## Variables
graphCodeTitle <- "ID_Expression.R"
outGraph <- "../analysis/graphs/ID_Expression/DS2-11_FtMm250_200-3sdgd_Mt5_RegNumiLibBrain_KeepCC_PC1to40/ID_Expression_"
outTable <- "../analysis/tables/ID_Expression/DS2-11_FtMm250_200-3sdgd_Mt5_RegNumiLibBrain_KeepCC_PC1to40/ID_Expression_"
...
# Heatmap + hclust
pg <- Seurat_Heatmap_By_Cluster_Hclust_Genes(genes = id_genes
  , clusterOrder = c(9,7,8,10,2,0,1,4,3,13,5,6,11,12,14,15))
# Title
title = paste0(graphCodeTitle
  , "\n\nExpression of de novo ID NEJM + Lancet genes"
  , "\nx-axis: Genes"
...
# Subset to Epilepsy
genes_DF <- genes_DF[genes_DF$Gene %in% id_genes, ]
genes <- rev(genes_DF$Gene)
genes <- c(as.character(id_genes)[! id_genes %in% genes], as.character(genes))
...
write.table(rev(genes)
  , file = paste0(outTable
    , "EnrichmentSorted_HeatmapSetColWid_NormCenterScale_genes.txt"
  )
...
ggsave(paste0(outGraph
    , "Lake_EnrichmentSorted_HeatmapSetColWid_NormCenterScale_paper.png"
  )
  , width = 11, height = 8
)
...
ggsave(paste0(outGraph
    , "LakeDropseq_EnrichmentSorted_HeatmapSetColWid_NormCenterScale_paper.png"
  )
  , width = 16, height = 8
)
...
gene_binary_M <- Add_Gene_List_To_Binary_Matrix(
  gene_binary_M, geneGroupDF$Gene, "ID")
# Format
...
ggsave(paste0(
  outGraph, "EnrichmentSorted_DiseaseIntersection.pdf")
  , width = 4, height = 6)
################################################################################
