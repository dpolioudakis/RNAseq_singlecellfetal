...
## Variables
graphCodeTitle <- "Seurat_Cluster_DS-2-3-4-5-6-7.R"
outGraphPfx <- "../analysis/graphs/Seurat_Cluster_DS-2-3-4-5-6-7/Seurat_Cluster_DS-2-3-4-5-6-7_exon_FtMm250_RegLibBrain_"
outTablePfx <- "../analysis/tables/Seurat_Cluster_DS-2-3-4-5-6-7/Seurat_Cluster_DS-2-3-4-5-6-7_exon_FtMm250_RegLibBrain_"
outRdatPfx <- "../analysis/Seurat_Cluster_DS-2-3-4-5-6-7/Seurat_Cluster_DS-2-3-4-5-6-7_exon_FtMm250_RegLibBrain_"
...
centSO <- Setup(centSO, min.cells = 3, min.genes = 200
  , do.logNormalize = TRUE, total.expr = 1e4, project = "DS-2-3-4-5-6-7"
...
br3Cells <- c(as.character(metDF$CELL)[metDF$SEQ_RUN == "DS-005-006" | metDF$SEQ_RUN == "DS-007"])
...
dhgCells <- c(as.character(metDF$CELL)[metDF$SEQ_RUN == "DS-005-006" & metDF$NEXTERA == "N701"]
  ...
  , as.character(metDF$CELL)[metDF$SEQ_RUN == "DS-007" & metDF$NEXTERA == "N710"]
  , as.character(metDF$CELL)[metDF$SEQ_RUN == "DS-007" & metDF$NEXTERA == "N711"])
...
pdf(paste0(outGraphPfx, "tSNE_Metadata_200gd_2500dg_Mt5_PCA1to15.pdf")
  , width = 8, height = 6)
ggC; ggP; ggB; ggR; ggS; ggL
dev.off()
# Save as grid
pdf(paste0(outGraphPfx, "tSNE_MetadataGrid_200gd_2500dg_Mt5_PCA1to15.pdf")
  , width = 12, height = 16)
...
ldf <- lapply(1:20, function(pc) {
  data.frame(GENES = names(sort(abs(centSO@pca.obj[[1]]$rotation[ ,pc]), decreasing = TRUE)[1:10])
    , PC = paste0("PC", pc))
})
...
ggDF <- ggDF[idx, ]
row.names(ggDF) <- paste0(length(ggDF$GENES):1, "_", ggDF$GENES)
ggDF <- ggDF[ ,-c(1:2)]
# Order by clustering
...
# Add PCs
idx <- match(ggDF$Var1, paste0(length(genesDF$GENES):1, "_", genesDF$GENES))
ggDF$PC <- genesDF$PC[idx]
# Set sample order by clustering
ggDF$Var2 <- factor(ggDF$Var2, levels = names(sort(centSO@ident)))
# Set expression limits
ggDF$value[ggDF$value > 5] <- 5
# ggplot
...
ggsave(paste0(outGraphPfx, "DoHeatmap_Top10Markers_CenterScale.png")
  , width = 10, height = 4 + 0.13*nrow(top10), dpi = 600)
