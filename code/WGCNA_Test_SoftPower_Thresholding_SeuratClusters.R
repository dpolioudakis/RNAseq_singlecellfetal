...
print(args)
if (length(args) == 0) {
  print("No arguments supplied, setting clusterID to 1")
  clusterID <- 1
} else {
...
# Variables
outGraph <- paste0("../analysis/graphs/WGCNA/DS2-11/Test_SoftPower_Threshold_SeuratCluster/WGCNA_Test_SoftPower_Threshold_SeuratCluster_", clusterID, "_")
outAnalysis <- paste0("../analysis/analyzed_data/WGCNA/DS2-11/Test_SoftPower_Threshold_SeuratCluster/WGCNA_Test_SoftPower_Threshold_SeuratCluster_", clusterID, "_")
graphCodeTitle <- "WGCNA_Test_SoftPower_Threshold_SeuratCluster.R DS2-11"
...
# Subset to cluster
cellIDs <- names(centSO@ident)[centSO@ident %in% clusterID]
exM <- noCentExM[colnames(noCentExM) %in% cellIDs, ]
# Mean expression of each gene to use for filtering
mnEx <- sort(rowMeans(exM), decreasing = TRUE)
# Check top 10 genes sorted by mean expression
print(head(mnEx))
